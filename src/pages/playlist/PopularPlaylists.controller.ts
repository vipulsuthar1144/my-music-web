...
const usePopularPlaylistsController = () => {
  ...
  const { isPopularPlaylistsError, isPopularPlaylistsLoading, hasMorePopularPlaylists, popularPlaylists, popularPlaylistsOffset } = useAppSelector((state) => state.playlist);
  ...
  const handleGetPlaylistsByCategoryId = () => {
    dispatch(getPopularPlaylists({ limit: 20, offset: popularPlaylistsOffset }));
  };
  const lastPlaylistListItemRef = useLoadMore(handleGetPlaylistsByCategoryId, isPopularPlaylistsLoading, hasMorePopularPlaylists, isPopularPlaylistsError);
